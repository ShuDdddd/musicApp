      ...
      <div class="song_box">
        <div class="song_info">
          ...
          <div class="circle"
               ...
               id="circle"></div>
          <div class="show_time">{{currentTime | timeFormat}}/{{ listArr[current].totalTime | timeFormat}}</div>
        </div>
      </div>
    </div>
    <!--音量-->
  ...
  methods: {
    ...
    play () {
      ...
      this.progressTimer = window.setInterval(() => {
        ...
        this.circleLeft = Math.round(this.currentTime / this.listArr[this.current].totalTime * 400) - 8
        this.progressGo.css('width', percentage(this.currentTime, this.listArr[this.current].totalTime) + '%')
    ...
    // 进度条小球
    moveCircle (e) {
      if (!this.listArr[this.current].totalTime || !e.buttons) {
        return
      }
      ...
      if (x >= -8 && x <= 392) {
        ...
        this.currentTime = Math.round((x + 8) / 400 * this.listArr[this.current].totalTime)
        this.audio.currentTime = this.currentTime
      }
    },
    // 点击进度条小球移动到该位置
    downProgress (e) {
      if (!this.listArr[this.current].totalTime) {
        return
      }
