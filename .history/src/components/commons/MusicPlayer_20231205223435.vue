<template>
  <div class="music_player_box">
    <div class="operation">
      ...
      <!--暂停播放模块-->
      <div class="stop">
        <audio id="myAudio" :src="listArr[current].musicUrl"></audio>
    ...
    <div class="message">
      <div class="img_box">
        <img :src="listArr[current].picUrl ? listArr[current].picUrl : '@/assets/img/2.png'"
             @click="toSong"
             :onerror='defaultImg'>
      </div>
      <div class="song_box">
        <div class="song_info">
          <a class="song_name">{{listArr[current].name}}</a>
          <a class="singer">{{listArr[current].singer}}</a>
        </div>
          ...
          <div class="circle"
               ...
               id="circle"></div>
          <div class="showTime">{{currentTime | timeFormat}}/{{totalTime | timeFormat}}</div>
        </div>
      </div>
    </div>
    <!--音量-->
...
export default {
  ...
  methods: {
    ...
    async getMusicMes (id) {
      ...
      if (res.code === 200) {
        this.musicMes.id = id
        this.musicMes.name = res.songs[0].name
        this.musicMes.picUrl = res.songs[0].al.picUrl
        this.musicMes.singer = res.songs[0].ar[0].name
        this.musicMes.singerId = res.songs[0].al.id
        this.musicMes.mvId = res.songs[0].mv
      }
      const { data: url } = await this.$http.get('song/url', { params: { id: id } })
      if (url.code === 200) {
        this.musicMes.musicUrl = url.data[0].url
      }
    },
    // 获取音乐时长
    getTotalTime () {
      if (!this.listArr[this.current].musicUrl) {
        this.totalTime = 0
        return
      }
    ...
    play () {
      ...
      this.progressTimer = window.setInterval(() => {
        ...
        this.circleLeft = Math.round(this.currentTime / this.totalTime * 400) - 8
        this.progressGo.css('width', percentage(this.currentTime, this.totalTime) + '%')
    ...
    toSong () {
      this.$router.push({
        name: 'song',
        params: { id: this.listArr[this.current].id }
      })
    },
    // 音乐暂停
    stop () {
    ...
    async preOrNext (num) {
      ...
      this.current += num
      if (this.current >= this.listArr.length) this.current = 0
      else if (this.current < 0) this.current = this.listArr.length - 1
      this.currentTime = 0 // 当前音乐播放进度为0
    ...
    // 音乐列表添加音乐
    async addMusic (arr) {
      if (this.listArr.length === 1 && this.listArr[0].id == null) {
        this.listArr.shift()
      }
    ...
    moveCircle (e) {
      ...
      if (x >= -8 && x <= 392) {
        ...
        this.currentTime = Math.round((x + 8) / 400 * this.totalTime)
