  ...
  <el-dialog
    ...
    <div v-if="loginWay === 'ewm'">
      ...
      <div class="erweima_box">
        ...
        <div class="img_cover" v-if="isExpire">
          <el-button @click="getErweima">二维码过期点击刷新</el-button>
        </div>
      </div>
      <p>使用网易云app扫码登录</p>
      <el-button @click="changeLoginWay('mm')">使用其他方式登录</el-button>
    </div>
    <el-form :model="telLoginForm"
      ...
      <el-form-item>
        <el-link type="info" :underline="false"
                 v-if="loginWay === 'yzm'" @click="changeLoginWay('mm')">
          <div class="switch_btn">密码登录</div>
        </el-link>
        <el-link type="info" :underline="false"
                 v-if="loginWay === 'mm'" @click="changeLoginWay('yzm')">
      ...
      <el-button @click="changeLoginWay('ewm')">使用二维码扫码登录</el-button>
    </el-form>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    changeLoginWay (way) {
      this.loginWay = way
      // 二维码登录需要开启计时器检测二维码是否过期,其他登录方式关闭定时器
      if (way === 'ewm') {
        if (!this.erweima) {
          this.getErweima()
        } else {
          this.timer = setInterval(this.checkErweima, 1000)
        }
      } else {
